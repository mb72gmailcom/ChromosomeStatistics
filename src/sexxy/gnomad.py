...
def gnomad_af_key(chrom: str, pos: str | int, ref: str, alt: str) -> str:
    """Return ``chrN_pos_ref_alt`` lookup key used in gnomAD JSON files."""
    return f"{chrom}_{pos}_{ref}_{alt}"


def load_gnomad_af_json(path: str | Path) -> dict[str, float]:
    """Load a gnomAD common-AF JSON file (``chrN_pos_ref_alt`` -> frequency)."""
...
class GnomadAfStore:
    ...
    Each file is a JSON object mapping ``chrN_pos_ref_alt`` keys to AF.
    """
    ...
    def get(self, chrm: str, variant_key: str) -> float:
        """Return AF for *variant_key* (``chrN_pos_ref_alt``), default ``0``."""
